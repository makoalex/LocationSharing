...
export default function ChatButton({socketId, username}:ChatButtonProps) {
  ...
  return (
    <div className=' hover:bg-accent'>
        <img src={chatIcon} alt=" chat icon" onClick={handleChatBox} className= 'absolute right-2 top-2 rounded-sm  w-[50px] h-[40px]  bg-black shadow-[4px_4px_0px_0px_#000] active:shadow-none active:transition-all active:duration-300 cursor-pointer'/>
    </div>
  )
}
